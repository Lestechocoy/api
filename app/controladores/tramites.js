...
/******************************************************************************************************
                 CONTROLADOR PARA OBTENER UN TRAMITE POR SU ID
 *****************************************************************************************************/
                 async function getEstadoTramiteByCui(req, res) {
                    try {
                        const cui = req.params.cui; // Se espera que el ID del NOTA esté en los parámetros de la URL
                        const estado = await tramitesModel.getEstadoTramitebyCui(cui)
                        if (!estado) {
                            return respuesta.NotFound(res, req, 'NOTA no encontrado', 400)
                        }
