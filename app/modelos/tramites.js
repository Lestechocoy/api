...
/******************************************************************************************************
                 FUNCION PARA CREARUN RECIBO
 *****************************************************************************************************/

async function createTramite(clienteData, servicioData) {

    return new Promise(async (resolve, reject) => {
        const { nombre, direccion, telefono, cui, fechaActual, horaActual } = clienteData;
        try {
            const clienteQuery = "INSERT INTO clientes_tramites (Nombre, Direccion, Telefono, Cui, Fecha, Hora) VALUES (?, ?, ?, ?, ?, ?)";
            const clienteResult = await mysqlConnection.query(clienteQuery, [nombre, direccion, telefono, cui, fechaActual, horaActual]);

            const Tramitesquery = "INSERT INTO detalles_tramites (IdClientesTramites, Descripcion, Estado) VALUES (?, ?, ?)";
            ...
            if (Array.isArray(servicioData)) {
                await Promise.all(servicioData.map(async (servicio) => {
                    ...
                    try {
                        await mysqlConnection.query(Tramitesquery, [clienteResult.insertId, descripcion, estado]);
...
async function updateTramiteById(IdCliente, updatedTramiteData) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const promesasConsultas = updatedTramiteData.map(async (updatedTramiteData) => {
                const { IdTramite, Descripcion, Estado } = updatedTramiteData;
                const queryDetallesTramites = "SELECT * FROM detalles_tramites WHERE Id = ? AND IdClientesTramites = ?";

                const detallesTramitesResults = await mysqlConnection.query(queryDetallesTramites, [IdTramite, IdCliente]);
 ...
 *****************************************************************************************************/
                 async function getEstadoTramitebyCui(cui) {
                    return new Promise(async (resolve, reject) => {
                        ...
                        try {
                            // Ejecutar la consulta SQL
                            const rows = await mysqlConnection.query(query, [cui]);
                            if (rows.length === 0) {
                                reject(500)
                                return;
                            }
