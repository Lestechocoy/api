...
const codigo = require('../Helpers/RandomText');
...
async function createRecibo(clienteData, servicioData) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const facturaResult = await mysqlConnection.query(facturaquery, [clienteResult.insertId, corr, codigoRecibo, fechaActual, horaActual, estadorecibo, cantidadAnticipada, saldoPendiente]);
            ...
            //si el anticipo es igual al total de productos actualizar los datos para cancelado

            // const sumaTotal = `SELECT
            ...
            // mysqlConnection.query(sumaTotal, [correlativo], (error, results) => {
            //     if (error) {
            //         console.error('Error al obtener el total del recibo:', error);
            //     } else {
            //         const total = results[0].SumaPrecioTotal;
            //         console.log('El total del recibo es:', total);

            //         if (total == cantidadAnticipada) {
